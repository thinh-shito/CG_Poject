...
function createScene() {
	...
	scene.fog = new THREE.Fog(0xF2F8F7, 100, 950);
	camera.position.x = 0;
...
Sky = function () {
	...
	for (var i = 0; i < this.nClouds; i++) {
		...
		this.clouds.push(c);
		a = stepAngle * i;
		h = game.seaRadius + 150 + Math.random() * 200;
	...
	for (var j = 0; j < this.nStars; j++) {
		a = stepAngle * j;
		h = game.seaRadius + 250 + Math.random() * n;
		c = new Star();
		c.mesh.position.y = Math.sin(a) * h;
		c.mesh.position.x = Math.cos(a) * h;
...
function loop() {
	...
	if (onChange && game.status == "waiting") {
		...
		if (camera.position.x <= 0 && camera.position.x != -50) camera.position.x -= 0.5;
	} else {
		if (camera.position.z > 200) camera.position.z -= 1;
		else if (camera.position.z < 200) camera.position.z += 1;
		if (camera.position.x < 0)camera.position.x += 0.5;
	}
...
function createGui() {
	...
	gui.add(
		option,
		"Airplane_parts",
		["None"].concat(Object.keys(planeSetting))
